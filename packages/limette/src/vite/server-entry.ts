...
export function generateServerEntry(
  options: GenerateServerEntryOptions,
): string {
  ...
  for (const routeId of assets.keys()) {
    if (!knownRouteIds.has(routeId)) {
      throw new Error(
        `Server entry assets contain unknown Limette route ID "${routeId}".`,
      );
    }
  }
  ...
  const routeDefinitions = manifest.routes.map((route) => {
    ...
    if (
      route.islandImports.length > 0 &&
      (!routeAssets || routeAssets.scripts.length === 0)
    ) {
      throw new Error(
    ...
    return [
      ...
      `    layouts: [${
        route.layouts.map((path) => modules.get(path)).join(', ')
      }],`,
      `    middlewares: [${
        route.middlewares.map((path) => modules.get(path)).join(', ')
      }],`,
      `    islands: ${
  ...
  const moduleImports = Array.from(
    modules,
    ([path, localName]) =>
      `import * as ${localName} from ${jsLiteral(resolve(root, path))};`,
  );
  ...
  return [
    ...
    ...moduleImports,
    `import { registerRouteDefinitions } from ${
      jsLiteral(options.runtimeModule ?? SERVER_RUNTIME_MODULE_ID)
    };`,
    '',
    'const routes = [',
    ...routeDefinitions,
