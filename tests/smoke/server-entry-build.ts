...
async function buildFixture(cwd: string) {
  const command = viteCommand(['--config', join(fixtureRoot, 'vite.config.ts'), 'build'], { cwd });
...
try {
  ...
  const clientManifest = JSON.parse(await Deno.readTextFile(clientManifestPath)) as Record<
    string,
  ...
  const aboutRoute = routeManifest.routes.find((route) => route.path === '/about');
  const csrRoute = routeManifest.routes.find((route) => route.path === '/csr-islands');
  ...
  assert(homeAssets && aboutAssets && csrAssets, 'Missing resolved CSS fixture assets.');
  assert(
  ...
  const cssContents = new Map(
    await Promise.all(
      emittedCssFiles.map(async (path) => [path, await Deno.readTextFile(path)] as const)
    )
  );
  ...
  const sharedCssUrl = `/my-app/${sharedCssFiles[0]
    .slice(clientDir.length + 1)
    .replaceAll('\\', '/')}`;
  assert(
    homeAssets.styles.includes(sharedCssUrl) && aboutAssets.styles.includes(sharedCssUrl),
    'Shared CSS was not associated with every affected route.'
  );
  const routeCss = async (styles: readonly string[]) =>
    (
      await Promise.all(
        styles.map((url) => Deno.readTextFile(join(clientDir, url.slice('/my-app/'.length))))
      )
    ).join('\n');
  ...
  for (const marker of ['fixture-island-style', 'fixture-shared-island-style']) {
  ...
  for (const marker of ['fixture-status-island-style', 'fixture-shared-island-style']) {
  ...
  const sharedIslandCssUrl = `/my-app/${sharedIslandCssFiles[0]
    .slice(clientDir.length + 1)
    .replaceAll('\\', '/')}`;
  assert(
    counterStyles.includes(sharedIslandCssUrl) && statusStyles.includes(sharedIslandCssUrl),
    'Shared island CSS was not associated with both islands.'
  );
  assert(
    JSON.stringify([...repositoryRootAssets]) === JSON.stringify([...fixtureRootAssets]),
    'Route assets changed when Vite was invoked from another working directory.'
  );
  const homeEntry = Object.values(clientManifest).find(
    (chunk) => chunk.isEntry && chunk.name === `limette-route-${homeRoute.id}`
  );
  ...
  const homeEntrySource = await Deno.readTextFile(join(clientDir, homeEntry.file));
  assert(
  ...
  const browserRegistration = await new Deno.Command('node', {
    ...
    args: [
      ...
      `
        ...
        await import(${JSON.stringify(pathToFileURL(join(clientDir, homeEntry.file)).href)});
  ...
  assert(
    ...
    `The production island did not register and upgrade:\n${new TextDecoder().decode(
      browserRegistration.stderr
    )}`
  );
  const csrEntry = Object.values(clientManifest).find(
    (chunk) => chunk.isEntry && chunk.name === `limette-route-${csrRoute.id}`
  );
  ...
  const csrBrowserRegistration = await new Deno.Command('node', {
    ...
    args: [
      ...
      `
        ...
        await import(${JSON.stringify(pathToFileURL(join(clientDir, csrEntry.file)).href)});
  ...
  assert(
    expectedStyleUrls.length > 0 &&
      (
        await Promise.all(
          expectedStyleUrls.map((url) => exists(join(clientDir, url.slice('/my-app/'.length))))
        )
      ).every(Boolean),
  ...
  const homeResponse = await serverModule.handler(new Request('http://localhost/'));
  ...
  assert(
    homeHtml.includes('<title>Generated home title</title>') &&
      homeHtml.includes('<meta name="description" content="Generated app description">'),
    'Generated production entry did not merge structural head metadata.'
  );
  assert(homeHtml.includes('data-server-layout'), 'Home layout did not render.');
  assert(
  ...
  const islandShadow = (tagName: string) => {
    const match = homeHtml.match(new RegExp(`<${tagName}[^>]*>([\\s\\S]*?)<\\/${tagName}>`));
  ...
  assert(
    statusStyles
      .filter((url) => !counterStyles.includes(url))
      .every((url) => !counterShadow.includes(url)) &&
      counterStyles
        .filter((url) => !statusStyles.includes(url))
        .every((url) => !statusShadow.includes(url)),
    "An island shadow root received another island's stylesheet."
  );
  const pageOnlyStyleUrls = expectedStyleUrls.filter(
    (url) => !counterStyles.includes(url) && !statusStyles.includes(url)
  );
  assert(
    pageOnlyStyleUrls.length > 0 &&
      pageOnlyStyleUrls.every((url) => !counterShadow.includes(url) && !statusShadow.includes(url)),
    'Document-only CSS was injected into an island shadow root.'
  );
  assert(
    homeHtml.includes('shadowroot="open"') || homeHtml.includes('shadowrootmode="open"'),
    'Generated island metadata was not used by SSR.'
  );

  const aboutResponse = await serverModule.handler(new Request('http://localhost/about'));
  ...
  const userResponse = await serverModule.handler(new Request('http://localhost/users/alice'));
  ...
  assert(
    userHtml.includes('<title>User profile</title>') &&
      userHtml.includes('<meta name="description" content="Profile for alice">'),
  ...
  const newUserResponse = await serverModule.handler(new Request('http://localhost/users/new'));
  const newUserHtml = await newUserResponse.text();
  assert(
    newUserResponse.status === 200 &&
      newUserHtml.includes('New user') &&
  ...
  const csrResponse = await serverModule.handler(new Request('http://localhost/csr-islands'));
  ...
  assert(
    csrAssets.styles.every((url) => csrHtml.includes(url)) &&
      Object.values(csrAssets.islandStyles)
        .flat()
        .every((url) => csrHtml.includes(url)),
