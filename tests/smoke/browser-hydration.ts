...
async function waitForServer(origin: string, child: Deno.ChildProcess) {
  ...
  throw new Error(
    `Server did not start: ${lastError instanceof Error ? lastError.message : String(lastError)}`
  );
}
...
async function verifyBfcacheRecovery(
  ...
  assert(changedRoute !== originalRoute, 'BFCache route fixture did not change.');
  ...
  const state = () =>
    page.evaluate(() => ({
      ...
      restores: Number(sessionStorage.getItem('__limetteBfcacheRestores') ?? '0'),
    }));

  try {
    await page.goto(`${origin}/`, { waitUntil: 'load' });
    assert((await state()).loads === 1, 'Initial dev page loaded more than once.');
    assert(
    ...
    if (restored.restores === 0) {
      ...
      await page.waitForFunction(
        (loads) => Number(sessionStorage.getItem('__limetteBfcacheLoads') ?? '0') > loads,
    ...
    assert(restored.restores === 1, 'BFCache restoration was handled more than once.');
    console.log(
    ...
    assert((await state()).loads === stableLoadCount, 'BFCache recovery entered a reload loop.');
...
async function verifyMixedPage(
  ...
  const instrumentedHtml = serverHtml.replace(entryScript, `${marker}${entryScript}`);
  ...
  assert(
    ...
    `${label} executed the CSR-only component before browser bootstrap: ${JSON.stringify(
      beforeBootstrap
    )}.`
  );
  ...
  try {
    ...
    const state = await page.evaluate(() => ({
      ...
      csrText: document.querySelector('test-client-only')?.shadowRoot?.textContent,
    }));
    throw new Error(
  ...
  const hydrated = await page.evaluate(() => {
    ...
    return {
      ssrHosts: document.querySelectorAll('test-counter').length,
      ssrCountTexts: [...(ssrHost?.shadowRoot?.querySelectorAll('span') ?? [])].filter((node) =>
        node.textContent?.includes('Count:')
      ).length,
      csrHosts: document.querySelectorAll('test-client-only').length,
      csrCountTexts: [...(csrHost?.shadowRoot?.querySelectorAll('span') ?? [])].filter((node) =>
        node.textContent?.includes('Client count:')
      ).length,
      reusedSsrNode:
        (window as typeof window & { __limetteSsrButton?: Element }).__limetteSsrButton ===
        firstSsrButton,
      deferHydration: ssrHost?.hasAttribute('defer-hydration'),
      litElementRuntimeCount: (globalThis as typeof globalThis & { litElementVersions?: string[] })
        .litElementVersions?.length,
      litHtmlRuntimeCount: (globalThis as typeof globalThis & { litHtmlVersions?: string[] })
        .litHtmlVersions?.length,
    };
  });
  assert(
    hydrated.ssrHosts === 1 && hydrated.ssrCountTexts === 1,
    `${label} duplicated the SSR island during hydration: ${JSON.stringify(
      hydrated
    )}. Errors: ${browserErrors.join('\n')}`
  );
  assert(hydrated.reusedSsrNode, `${label} replaced instead of hydrating SSR DOM.`);
  assert(
    hydrated.deferHydration === false,
    `${label} left defer-hydration on the hydrated island.`
  );
  assert(
    hydrated.litElementRuntimeCount === 1 && hydrated.litHtmlRuntimeCount === 1,
  ...
  const interaction = await page.evaluate(async () => {
    ...
    const csr = document.querySelector('test-client-only') as
      | (HTMLElement & { updateComplete?: Promise<unknown> })
      | null;
    (ssr?.shadowRoot?.querySelectorAll('button')[1] as HTMLButtonElement | undefined)?.click();
    (csr?.shadowRoot?.querySelectorAll('button')[1] as HTMLButtonElement | undefined)?.click();
    await Promise.all([ssr?.updateComplete, csr?.updateComplete]);
    return {
  ...
  assert(
    interaction.ssrText?.includes('Count: 1') && interaction.csrText?.includes('Client count: 1'),
    `${label} islands were not both interactive: ${JSON.stringify(interaction)}.`
  );
...
async function stop(child: Deno.ChildProcess | undefined, output: Promise<string> | undefined) {
  if (!child) return;
  try {
    child.kill('SIGTERM');
  } catch {
...
try {
  ...
  devChild = viteCommand(
    ...
    { cwd: fixtureRoot, stderr: 'inherit' }
  ).spawn();
  ...
  await verifyMixedPage(devPage, devOrigin, '/@limette/client-entry/', 'Vite dev', devHtml);
  await devPage.close();
  ...
  const build = await viteCommand(['--config', join(fixtureRoot, 'vite.config.ts'), 'build'], {
    cwd: fixtureRoot,
  }).output();
  if (!build.success) {
    throw new Error(new TextDecoder().decode(build.stderr));
  }
  await Deno.copyFile(join(fixtureRoot, 'deno-runner.ts'), join(buildDir, 'deno-runner.ts'));
  ...
  const productionHtml = await waitForServer(productionOrigin, productionChild);
