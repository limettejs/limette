...
try {
  ...
  const styleUrls = [
    ...aboutHtml.matchAll(/<link rel="stylesheet" href="([^"]+)"/g),
  ]
    .map((match) => match[1]);
  assert(
  ...
  const changedCss = originalCss.replace(
    'fixture-source: about',
    'fixture-source: about-hmr',
  );
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    servedCss = await (
      await fetch(`${origin}/styles/about.css?direct`)
    ).text();
    if (servedCss.includes('about-hmr')) break;
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  assert(
    servedCss.includes('about-hmr'),
    'Vite served stale CSS after editing.',
  );
  await Promise.race([
    cssUpdate,
    new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error('CSS-only edit did not emit an HMR update.')),
        5_000,
      )
    ),
  ]);
  ...
  assert(
    ...
    'Clientless route reload fixture did not change.',
  );
  ...
  await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(
        () =>
          reject(new Error('Clientless route edit did not emit full reload.')),
        5_000,
      )
    ),
  ]);
