...
function assertMissing(name: string, graph: string, forbidden: readonly string[]) {
  for (const dependency of forbidden) {
    assert(!graph.includes(dependency), `${name} unexpectedly reaches ${dependency}.`);
  }
}

const neutralForbidden = ['"node:', '/src/node.ts', '/src/deno.ts', '/src/vite/', 'npm:vite'];
const coreGraph = await moduleGraph('packages/limette/src/mod.ts');
const serverRuntimeGraph = await moduleGraph('packages/limette/src/server-runtime.ts');
assertMissing('limette', coreGraph, neutralForbidden);
assertMissing('limette/internal/server-runtime', serverRuntimeGraph, neutralForbidden);
...
assertMissing('limette/node', nodeGraph, ['/src/deno.ts', '/src/vite/', 'npm:vite']);

const denoGraph = await moduleGraph('packages/limette/src/deno.ts');
assertMissing('limette/deno', denoGraph, ['"node:', '/src/node.ts', '/src/vite/', 'npm:vite']);
...
assertMissing('limette/vite', viteGraph, ['/src/node.ts', '/src/deno.ts']);
