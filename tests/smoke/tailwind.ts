...
async function stopDevServer(process: ReturnType<typeof startVite>) {
...
function hasDeclaration(css: string, property: string, value: string) {
  return new RegExp(`${property}:\\s*${value.replaceAll('[', '\\[')}`).test(css);
}
...
async function waitForCss(origin: string, path: string, property: string, value: string) {
  let css = '';
  for (let attempt = 0; attempt < 80; attempt++) {
    const response = await fetch(`${origin}${path}`);
...
try {
  ...
  const build = await runVite(['--config', join(fixtureRoot, 'vite.config.ts'), 'build']);
  assert(build.success, `Tailwind fixture build failed:\n${build.stderr}`);
  ...
  assert(updatedRoute !== originalRoute, 'Route update fixture did not change.');
  ...
  const routeCss = await waitForCss(origin, routeDevStyle, 'padding', '19px');
  assert(
    !hasDeclaration(routeCss, 'padding', '13px'),
    'Tailwind retained the stale route candidate after an edit.'
  );

  const updatedIsland = originalIsland.replace('outline-[7px]', 'outline-[9px]');
  assert(updatedIsland !== originalIsland, 'Island update fixture did not change.');
  ...
  const islandCss = await waitForCss(origin, islandDevStyle, 'outline-width', '9px');
  assert(
