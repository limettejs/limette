...
async function verifyStaticHandler(name: string, handler: StaticHandler) {
  ...
  assert(
    asset.headers.get('content-type') === 'application/javascript; charset=utf-8',
    `${name} returned the wrong JavaScript content type.`
  );

  const head = await handler(new Request('http://localhost/assets/app.js', { method: 'HEAD' }));
  assert(
    head?.status === 200 && (await head.text()) === '',
    `${name} did not preserve HEAD behavior.`
  );
  assert(
    head.headers.get('content-type') === 'application/javascript; charset=utf-8',
    `${name} omitted the HEAD content type.`
  );

  assert(
    (await handler(new Request('http://localhost/assets/missing.js'))) === undefined,
    `${name} did not fall through for a missing file.`
  );
  assert(
    (await handler(new Request('http://localhost/assets/directory/'))) === undefined,
    `${name} served a directory as a file.`
  );
  assert(
    (await handler(new Request('http://localhost/outside/app.js'))) === undefined,
  ...
  assert(methodResponse?.status === 405, `${name} did not reject a non-static method.`);
...
try {
  ...
  await Deno.writeTextFile(join(staticRoot, 'app.js'), 'console.log("static");');
