...
const manifest: LimetteRouteManifest = {
  ...
  routes: [
    ...
    },
    {
      ...
      sourceFiles: ['routes/_app "quoted".ts', 'routes/island.ts', 'islands/test.ts'],
      styleImports: [],
    },
  ],
};
...
assert(
  first.indexOf('path: "/static"') < first.indexOf('path: "/island/\\"quoted\\""'),
  'Generated route definitions did not preserve manifest order.'
);
assert(
  occurrences(first, jsImport(join(options.root, 'routes/shared-layout.ts'))) === 1,
  'A shared layout was imported more than once.'
);
assert(
  occurrences(first, jsImport(join(options.root, 'routes/shared-middleware.ts'))) === 1,
  'Shared middleware was imported more than once.'
);
assert(
  first.includes('scripts: []') && first.includes('styles: ["/assets/static \\"quoted\\".css"]'),
...
assert(
  first.includes('app \\"quoted\\".ts"') && first.includes('_app \\"quoted\\".ts"'),
...
try {
  generateServerEntry({
    ...options,
    assets: new Map([...assets, ['unknown-id', { scripts: [], styles: [], islandStyles: {} }]]),
  });
} catch (error) {
...
try {
  await unresolvedAppPlugin.load.call(pluginContext, resolvedServerEntryId!);
} catch (error) {
  unresolvedAppError = error instanceof Error ? error.message : String(error);
}
assert(
  unresolvedAppError.includes('Could not resolve configured Limette app module "./missing-app.ts"'),
