...
async function emittedAsset(path = clientDir): Promise<string | undefined> {
  for await (const entry of Deno.readDir(path)) {
    ...
    if (entry.isDirectory) {
      ...
      entry.isFile && (extname(entry.name) === '.js' || extname(entry.name) === '.css')
    ) {
      return entryPath;
    }
  }
...
async function waitForWrangler(
  ...
  for (let attempt = 0; attempt < 150; attempt++) {
    ...
    try {
      ...
      if (
        response.status === 200 &&
        response.headers.get('x-limette-worker') === '1'
      ) {
...
try {
  ...
  const build = await viteCommand([
    '--config',
    join(fixtureRoot, 'vite.worker.config.ts'),
    'build',
  ...
  const expectedContentType = extname(assetPath) === '.css'
    ? 'text/css'
    : 'javascript';
  ...
  const missingAssetResponse = await fetch(
    `${origin}/assets/does-not-exist.js`,
  );
