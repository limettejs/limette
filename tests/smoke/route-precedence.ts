...
const missingAppRoot = await Deno.makeTempDir({
  prefix: 'limette-routes-missing-app-',
});
try {
  await Deno.mkdir(join(missingAppRoot, 'routes'), { recursive: true });
  ...
  await Deno.remove(missingAppRoot, { recursive: true });
}
