...
describe('imperative routing', () => {
  it('registers every HTTP method and ALL', async () => {
  ...
  it('implements method-first matching, HEAD fallback, and OPTIONS', async () => {
    const methodFirstApp = new App<TestState, TestPlatform>();
    methodFirstApp.error(
      '/*',
      (ctx) =>
  ...
  it('runs route-local, global, and ALL middleware in registration order', async () => {
  ...
  it('rejects equivalent duplicate registrations', () => {
    expectDuplicate((app) => {
  ...
  it('canonicalizes trailing slashes without rewriting internal path data', async () => {
