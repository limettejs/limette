...
async function withRoutes<T>(files: readonly string[], run: (root: string) => Promise<T>) {
...
function assertMatches(
  ...
  for (const pathname of matches) {
    assert(pattern.test({ pathname }), `Expected "${pathnamePattern}" to match "${pathname}".`);
  }
...
async function assertDiscoveryError(files: readonly string[], expected: readonly string[]) {
  let error: unknown;
  try {
...
describe('filesystem route manifest', () => {
  it('maps filesystem syntax to URLPattern paths', async () => {
    ...
    const mappings = new Map(mappingManifest.routes.map((route) => [route.routeFile, route.path]));
    for (const [file, expectedPath] of [
      ...
      assert(
        ...
        `Expected ${file} to map to ${expectedPath}, got ${mappings.get(file)}.`
      );
    }

    const nestedIndexManifest = await manifestFor(['blog/[slug]/comments/index.ts']);
    assert(
    ...
    assertMatches('/old/:path*', ['/old', '/old/one', '/old/one/two'], ['/older/one']);
  ...
  it('orders routes by specificity', async () => {
    ...
    const specificityPaths = specificityManifest.routes.map((route) => route.path);
    assertBefore(specificityPaths, '/users/new', '/users/:id');
    assertBefore(specificityPaths, '/users/new/:tab', '/users/:id/settings');
  ...
  it('rejects ambiguous and invalid route files', async () => {
    ...
    for (const duplicateParams of ['[id]/[id].ts', '[id]/[[id]].ts', '[[path]]/[...path].ts']) {
      await assertDiscoveryError(
        [duplicateParams],
        [
          duplicateParams,
    ...
    const classificationPaths = classificationManifest.routes.map((route) => route.path);
    for (const path of ['/my_layout', '/account_middleware', '/custom_app']) {
      assert(classificationPaths.includes(path), `${path} was treated as special.`);
    }
    assert(
      !classificationPaths.includes('/foo.d') && !classificationPaths.includes('/ignored.test'),
    ...
    await assertDiscoveryError(
      ...
      ['Duplicate middleware', 'nested/_middleware.js', 'nested/_middleware.ts']
    );
    await assertDiscoveryError(
      ['_app.js'],
    ...
    const missingAppRoot = await mkdtemp(join(tmpdir(), 'limette-routes-missing-app-'));
    try {
      await mkdir(join(missingAppRoot, 'routes'), { recursive: true });
      await writeFile(join(missingAppRoot, 'routes/index.ts'), 'export default class Route {}');
  ...
  it('produces deterministic route identities', async () => {
    ...
    assert(
      new Set(firstManifest.routes.map((route) => route.id)).size === firstManifest.routes.length,
