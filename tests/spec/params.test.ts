...
async function request(app: App, path: string) {
  const response = await app.handler()(new Request(`https://example.test${path}`));
...
type ContextParamsType = Assert<Equal<Context['params'], Readonly<Record<string, string>>>>;
...
describe('route parameters', () => {
  it('captures required, optional, catch-all, and encoded values', async () => {
    ...
    assert(
      (await capturedParam('/docs{/:version}?', '/docs/latest', 'version')) === 'latest',
    ...
    assert(
      (await capturedParam('/old/:path*', '/old/foo/bar', 'path')) === 'foo/bar' &&
        (await capturedParam('/old/:path*', '/old', 'path')) === '',
      'Catch-all route params did not preserve their slash-separated suffix.'
    );
    assert(
      (await capturedParam('/user/:id', '/user/hello%20world', 'id')) === 'hello world',
    ...
    assert(
      encodedSlash.text === 'foo%2Fbar' && structuralSlash.response.status === 404,
  ...
  it('handles malformed encoding and special parameter names safely', async () => {
    ...
    assert(
      (await request(specialNameApp, '/built-in/safe')).text === 'built-in:safe' &&
  ...
  it('keeps parameter dictionaries request-local within a request chain', async () => {
    ...
    staticApp.get('/about', (ctx) => {
      ...
      assert(
        Object.getPrototypeOf(ctx.params) === null && Object.keys(ctx.params).length === 0,
    ...
    assert(
      (await request(globalApp, '/users/123')).text === '123' && globalParams[0]?.id === '123',
  ...
  it('accumulates route-chain and explicit URLPattern parameters', async () => {
    ...
    const explicitResult = await explicitPatternApp.handler()(
      new Request('https://api.example.test/items/item-id?q=search-value#fragment')
    );
    assert(
      JSON.stringify(JSON.parse(await explicitResult.text())) === JSON.stringify({ id: 'item-id' }),
    ...
    assert(
      (await request(unnamedPatternApp, '/unnamed/natural-key')).text === 'natural-key',
  ...
  it('isolates parameters across concurrent requests', async () => {
    ...
    const concurrentIds = Array.from({ length: 100 }, (_, index) => `id-${index}`);
    const concurrentResults = await Promise.all(
      concurrentIds.map(async (id) => (await request(concurrentApp, `/concurrent/${id}`)).text)
    );
    assert(
      concurrentResults.every(
        (value, index) => value === `${concurrentIds[index]}:${concurrentIds[index]}`
      ),
